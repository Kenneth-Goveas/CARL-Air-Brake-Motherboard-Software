...
namespace cntrl :: local {
    ...
    std::string name_mpc_phrz = "prediction_horizon";
    std::string name_mpc_thrz = "termination_horizon";
...
int main (void) {
    ...
    cntrl::local::mpc_phrz = config::get<double>(cntrl::local::name_mpc_phrz);
    if (config::fail()) {
        logging::wrn(logging::local::mod,
            "Exiting"
        );
    ...
    cntrl::local::mpc_thrz = config::get<double>(cntrl::local::name_mpc_thrz);
    if (config::fail()) {
        logging::wrn(logging::local::mod,
            "Exiting"
        );
