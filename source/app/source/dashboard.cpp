...
namespace panel::local {
    bool inpt_err_lock = false, inpt_err_write = false, inpt_err_sntx = false, inpt_err_eof = false;
...
int main (int argc, char ** argv) {
    ...
    try {
        ...
        if (local::use_count != 1) {
            logging.err("Exactly one channel must be selected");
            throw local::except{};
        }
        ...
        panel::instance panel_inpt_head(terminal, input, output, terminal::color::darkblue, 2, 3, 1, 50),
                        panel_inpt_body(terminal, input, output, terminal::color::lightgray, 3, 3, 3, 50),
                        panel_inpt_field(terminal, input, output, terminal::color::darkgray, 4, 4, 1, 48),
                        panel_ctrl_head(terminal, input, output, terminal::color::darkblue, 7, 3, 1, 50),
                        panel_ctrl_body(terminal, input, output, terminal::color::lightgray, 8, 3, 3, 50),
                        panel_stat_head(terminal, input, output, terminal::color::darkblue, 7, 3, 1, 50),
                        panel_stat_body(terminal, input, output, terminal::color::lightgray, 8, 3, 9, 50),
                        panel_estm_head(terminal, input, output, terminal::color::darkblue, 7, 3, 1, 50),
                        panel_estm_body(terminal, input, output, terminal::color::lightgray, 8, 3, 6, 50),
                        panel_sens_head(terminal, input, output, terminal::color::darkblue, 7, 3, 1, 50),
                        panel_sens_body(terminal, input, output, terminal::color::lightgray, 8, 3, 19, 50);
        ...
        while (!interrupt.caught()) {
            ...
            panel_inpt_head.redraw();
            panel_inpt_body.redraw();
            panel_inpt_field.redraw();
            panel_inpt_head.put(terminal::color::white, 1, 2, "Input");
            ...
            if (panel::local::inpt_err_lock) {
                panel_inpt_body.put(terminal::color::darkred, 3, 2, "Channel lock error");
            } else if (panel::local::inpt_err_write) {
                panel_inpt_body.put(terminal::color::darkred, 3, 2, "Channel write error");
            } else if (panel::local::inpt_err_sntx) {
                panel_inpt_body.put(terminal::color::darkred, 3, 2, "Input syntax error");
            }

            panel::local::inpt_err_lock = false;
            panel::local::inpt_err_write = false;
            panel::local::inpt_err_sntx = false;
            panel::local::inpt_err_eof = false;

            try {
                if (local::use_ctrl) {
                    channel::local::ctrl = panel_inpt_field.get<double>(terminal::color::white, 1, 1);
                } else if (local::use_stat) {
                    channel::local::stat = panel_inpt_field.get<linalg::fvector>(terminal::color::white, 1, 1);
                } else if (local::use_estm) {
                    channel::local::estm = panel_inpt_field.get<linalg::fvector>(terminal::color::white, 1, 1);
                } else if (local::use_sens) {
                    channel::local::sens = panel_inpt_field.get<linalg::fvector>(terminal::color::white, 1, 1);
                }
            } catch (panel::instance::except_get except) {
                switch (except) {
                    case panel::instance::except_get::fail:
                        panel::local::inpt_err_sntx = true;
                        break;
                    case panel::instance::except_get::eof:
                        panel::local::inpt_err_eof = true;
                        break;
                }
            }

            if (panel::local::inpt_err_sntx) {
                continue;
            } else if (panel::local::inpt_err_eof) {
                break;
            }
            ...
            try {
                ...
                panel::local::inpt_err_lock = true;
            }
            ...
            try {
                ...
                panel::local::inpt_err_write = true;
            }
