...
void init (
    ...
    if (!(rad_acc >= -2048 && rad_acc <= 2048)) {
        logging::err(intern::mod,
            "Failed to initialize sensor #", id, " (Invalid accelerometer calibration radius)"
        );
        intern::fail = true;
        return;
    }

    if (!(rad_mag >= 144 && rad_mag <= 1280)) {
        logging::err(intern::mod,
            "Failed to initialize sensor #", id, " (Invalid magnetometer calibration radius)"
        );
        intern::fail = true;
        return;
    }

    if (!(off_acc >= linalg::ivector({-500, -500, -500}) && off_acc <= linalg::ivector({500, 500, 500}))) {
        logging::err(intern::mod,
            "Failed to initialize sensor #", id, " (Invalid accelerometer calibration offset)"
        );
        intern::fail = true;
        return;
    }

    if (!(off_mag >= linalg::ivector({-6400, -6400, -6400}) && off_mag <= linalg::ivector({6400, 6400, 6400}))) {
        logging::err(intern::mod,
            "Failed to initialize sensor #", id, " (Invalid magnetometer calibration offset)"
        );
        intern::fail = true;
        return;
    }

    if (!(off_gyr >= linalg::ivector({-2000, -2000, -2000}) && off_gyr <= linalg::ivector({2000, 2000, 2000}))) {
        logging::err(intern::mod,
            "Failed to initialize sensor #", id, " (Invalid gyroscope calibration offset)"
        );
        intern::fail = true;
        return;
    }
...
bool valid_rad_acc (int id) {
    bool flag;

    logging::inf(intern::mod,
        "Checking if sensor #", id, " accelerometer calibration radius is valid"
    );

    if (id != 1 && id != 2) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " accelerometer calibration radius is valid (Sensor does not exist)"
        );
        intern::fail = true;
        return flag;
    }
    ...
    if (!intern::valid_rad_acc[id - 1]) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " accelerometer calibration radius is valid (Reading unavailable)"
        );
        intern::fail = true;
        return flag;
    }

    flag = (intern::rad_acc[id - 1] >= -2048 && intern::rad_acc[id - 1] <= 2048);

    if (flag) {
        logging::inf(intern::mod,
            "Sensor #", id, " accelerometer calibration radius is valid"
        );
    } else {
        logging::inf(intern::mod,
            "Sensor #", id, " accelerometer calibration radius is invalid"
        );
    }
...
bool valid_rad_mag (int id) {
    bool flag;

    logging::inf(intern::mod,
        "Checking if sensor #", id, " magnetometer calibration radius is valid"
    );

    if (id != 1 && id != 2) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " magnetometer calibration radius is valid (Sensor does not exist)"
        );
        intern::fail = true;
        return flag;
    }
    ...
    if (!intern::valid_rad_mag[id - 1]) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " magnetometer calibration radius is valid (Reading unavailable)"
        );
        intern::fail = true;
        return flag;
    }

    flag = (intern::rad_mag[id - 1] >= 144 && intern::rad_mag[id - 1] <= 1280);

    if (flag) {
        logging::inf(intern::mod,
            "Sensor #", id, " magnetometer calibration radius is valid"
        );
    } else {
        logging::inf(intern::mod,
            "Sensor #", id, " magnetometer calibration radius is invalid"
        );
    }
...
bool valid_off_acc (int id) {
    bool flag;

    logging::inf(intern::mod,
        "Checking if sensor #", id, " accelerometer calibration offset is valid"
    );

    if (id != 1 && id != 2) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " accelerometer calibration offset is valid (Sensor does not exist)"
        );
        intern::fail = true;
        return flag;
    }
    ...
    if (!intern::valid_off_acc[id - 1]) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " accelerometer calibration offset is valid (Reading unavailable)"
        );
        intern::fail = true;
        return flag;
    }
    ...
    flag = (
           intern::off_acc[id - 1] >= linalg::ivector({-500, -500, -500})
        && intern::off_acc[id - 1] <= linalg::ivector({500, 500, 500})
    );

    if (flag) {
        logging::inf(intern::mod,
            "Sensor #", id, " accelerometer calibration offset is valid"
        );
    } else {
        logging::inf(intern::mod,
            "Sensor #", id, " accelerometer calibration offset is invalid"
        );
    }
...
bool valid_off_mag (int id) {
    bool flag;

    logging::inf(intern::mod,
        "Checking if sensor #", id, " magnetometer calibration offset is valid"
    );

    if (id != 1 && id != 2) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " magnetometer calibration offset is valid (Sensor does not exist)"
        );
        intern::fail = true;
        return flag;
    }
    ...
    if (!intern::valid_off_mag[id - 1]) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " magnetometer calibration offset is valid (Reading unavailable)"
        );
        intern::fail = true;
        return flag;
    }
    ...
    flag = (
           intern::off_mag[id - 1] >= linalg::ivector({-6400, -6400, -6400})
        && intern::off_mag[id - 1] <= linalg::ivector({6400, 6400, 6400})
    );

    if (flag) {
        logging::inf(intern::mod,
            "Sensor #", id, " magnetometer calibration offset is valid"
        );
    } else {
        logging::inf(intern::mod,
            "Sensor #", id, " magnetometer calibration offset is invalid"
        );
    }
...
bool valid_off_gyr (int id) {
    bool flag;

    logging::inf(intern::mod,
        "Checking if sensor #", id, " gyroscope calibration offset is valid"
    );

    if (id != 1 && id != 2) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " gyroscope calibration offset is valid (Sensor does not exist)"
        );
        intern::fail = true;
        return flag;
    }
    ...
    if (!intern::valid_off_gyr[id - 1]) {
        logging::err(intern::mod,
            "Failed to check if sensor #", id, " gyroscope calibration offset is valid (Reading unavailable)"
        );
        intern::fail = true;
        return flag;
    }
    ...
    flag = (
           intern::off_gyr[id - 1] >= linalg::ivector({-2000, -2000, -2000})
        && intern::off_gyr[id - 1] <= linalg::ivector({2000, 2000, 2000})
    );

    if (flag) {
        logging::inf(intern::mod,
            "Sensor #", id, " gyroscope calibration offset is valid"
        );
    } else {
        logging::inf(intern::mod,
            "Sensor #", id, " gyroscope calibration offset is invalid"
        );
    }
