...
linalg::fvector comp_sens (double time, double ctrl, linalg::fvector stat, linalg::fvector sens) {
    ...
    if (time >= rot_time + 1.0 / imu_rot_rate) {
    ...
    if (time >= qua_time + 1.0 / imu_qua_rate) {
    ...
    if (time >= lia_time + 1.0 / imu_lia_rate) {
    ...
    if (time >= pres_time + 1.0 / bar_pres_rate) {
    ...
    if (temp >= temp_time + 1.0 / bar_temp_rate) {
