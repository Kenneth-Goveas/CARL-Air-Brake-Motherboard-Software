...
void init (
    ...
    logging::inf(intern::mod,
        ...
        "Apog altd: ", mpc_apog, "m, Enab altd: ", mpc_enab, "m, ",
        "Time step: ", mpc_step, "s, Pred horz: ", mpc_phrz, "s, Term horz: ", mpc_thrz, "s, Tol: ", mpc_tol
    );
    ...
    if (mpc_thrz <= 0) {
        logging::err(intern::mod,
            "Failed to initialize control system (Invalid termination horizon)"
        );
        intern::fail = true;
        return;
    }
    ...
    intern::mpc_phrz = mpc_phrz;
    intern::mpc_thrz = mpc_thrz;
...
double get_ctrl (linalg::fvector estm) {
    ...
    if (pos_ver <= intern::mpc_enab) {
        ...
        if (intern::fail) {
            logging::err(intern::mod,
                "Failed to get control signal (Computation limit exceeded)"
            );
            intern::fail = true;
            return ctrl;
        }
